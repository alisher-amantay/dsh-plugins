...
/** Deployment policy for automatic continuation cadence. */
export interface Config {
  /** Delay before each automatic continuation round. */
  continuationDelayMs?: number
}

export const Config: z<Config> = z.object({
  continuationDelayMs: z.number().min(0).default(0),
})
...
export function apply(ctx: Context, config: Config = {}): void {
  const continuationDelayMs = config.continuationDelayMs ?? 0
  ...
  /** Release a pending cadence wait without leaving teardown blocked. */
  function cancelDelay(state: DriverState): void {
  ...
  /** Wait for the configured cadence under this exact driver lifecycle. */
  async function waitForCadence(state: DriverState): Promise<void> {
    if (continuationDelayMs === 0) return
  ...
  async function drive(state: DriverState): Promise<void> {
    ...
    if (continuationDelayMs > 0) {
      ...
      if (latest === undefined || latest.id !== goal.id || latest.revision !== goal.revision
        || latest.phase !== 'active' || latest.activation !== 'armed') return
    }
