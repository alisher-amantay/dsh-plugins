 ...
 * objective, a total-round-cap editor, and icon actions — resume when paused, edit (inline form in the
...
export interface GoalBarProps extends GoalBarActions {
  ...
  /** Highest admitted round number for the current goal. */
...
export function GoalBar({ goal, roundsStarted = 0, onEdit, onSetMaxRounds, onPause, onResume, onClear, t }: GoalBarProps & PropsLocale<'goal'>) {
  ...
  useEffect(() => {
    setRoundDraft(goal === undefined || goal === null ? '' : String(goal.maxGoalRounds))
  }, [goal?.maxGoalRounds])
  ...
  const roundCap = useMemo(() => {
    const value = Number(roundDraft)
    return Number.isSafeInteger(value) && value > 0 ? value : undefined
  }, [roundDraft])
  const handleSetMaxRounds = useCallback(async () => {
    if (roundCap === undefined) return
    await runAction(() => onSetMaxRounds(roundCap))
  }, [onSetMaxRounds, roundCap, runAction])
  ...
  return (
    <div className={css.dock} data-goal-bar>
      <div className={css.bar} title={title}>
        ...
        <label className={css.roundEditor}>
          <span>{t('rounds.label')}</span>
          <input
            className={css.roundInput}
            type="number"
            min="1"
            step="1"
            aria-label={t('rounds.aria')}
            value={roundDraft}
            onChange={(event) => { setRoundDraft(event.target.value) }}
            onKeyDown={(event) => {
              if (event.key === 'Enter') void handleSetMaxRounds()
            }}
          />
          <span>{t('rounds.progress', { roundsStarted })}</span>
          <button
            type="button"
            className={css.roundSave}
            disabled={pending || roundCap === undefined || roundCap === goal.maxGoalRounds}
            onClick={() => { void handleSetMaxRounds() }}
          >
            {t('rounds.apply')}
          </button>
        </label>
...
export function GoalDock({ useProjection, onEdit, onSetMaxRounds, onPause, onResume, onClear, t }: GoalDockProps) {
  const projection = useProjection('goal')
  return (
    <GoalBar
      goal={projection === undefined ? undefined : projection === null ? null : projection.goal}
      {...projection === undefined || projection === null ? {} : { roundsStarted: projection.roundsStarted }}
      onEdit={onEdit}
