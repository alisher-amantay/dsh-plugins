...
function makeActions() {
  return {
    ...
    onSetMaxRounds: vi.fn<GoalBarActions['onSetMaxRounds']>(() => Promise.resolve({ ok: true, value: undefined })),
...
describe('GoalBar', () => {
  ...
  it('lets the user replace an exhausted goal round cap with any larger total', async () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal({ maxGoalRounds: 4 })} roundsStarted={4} {...actions} t={t} />)

    expect(screen.getByText('已完成 4')).toBeTruthy()
    const input = screen.getByRole('spinbutton', { name: '目标总轮次上限' })
    fireEvent.change(input, { target: { value: '12' } })
    fireEvent.click(screen.getByRole('button', { name: '应用' }))

    await waitFor(() => { expect(actions.onSetMaxRounds).toHaveBeenCalledWith(12) })
  })

  it('rejects non-positive and fractional round caps before mutation', () => {
    const actions = makeActions()
    render(<GoalBar goal={makeGoal()} {...actions} t={t} />)
    const input = screen.getByRole('spinbutton', { name: '目标总轮次上限' })
    ...
    expect(actions.onSetMaxRounds).not.toHaveBeenCalled()
  })
