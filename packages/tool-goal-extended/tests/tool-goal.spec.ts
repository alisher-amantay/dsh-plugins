...
async function harness(config: toolGoal.Config = {}) {
  ...
  ctx.jobs.attachController('tool-goal-test')
...
describe('goal tool registration and presentation', () => {
  it('registers four exclusive tools plus configured guidance and disposes all contributions', async () => {
    const { ctx, fiber } = await harness({ blockedAfterConsecutiveRounds: 5 })
    const names = ['create_goal', 'defer_goal_until_job', 'get_goal', 'update_goal']
    ...
    expect(section?.text).toContain('defer_goal_until_job')
  ...
  it('has the Loader-safe namespace export shape', () => {
    ...
    expect(toolGoal.inject).toEqual(['agents', 'goals', 'jobs', 'tools', 'systemPrompt', 'sessionProjections'])
...
describe('goal tool state transitions', () => {
  ...
  it('lets an exact goal round defer itself until its blocking job settles', async () => {
    const { ctx, root } = await harness()
    const humanTurn = openTurn(root, { kind: 'user' })
    const created = ctx.goals.create(root.agent, { objective: 'finish after the evaluator' })
    closeTurn(root, humanTurn)
    ...
    const id = ctx.jobs.start({
      kind: 'bash',
      label: 'long evaluator',
      owner: root.agent,
      run: () => ({ cancel() {}, done }),
    })
    openTurn(root, { kind: 'goal', goalId: created.id, revision: created.revision, round: 1 })

    const deferred = await execute(ctx, 'defer_goal_until_job', {
      goal_id: created.id,
      revision: created.revision,
      job_id: id,
    }, root.agent)
    expect(resultGoal(deferred)).toMatchObject({ phase: 'active', revision: 1, roundsStarted: 1 })
    expect(resultJson(deferred)['activation']).toBe('disarmed')

    settle({ status: 'completed', output: 'all rows complete' })
    await ctx.jobs.wait(id, 1_000, root.agent)
