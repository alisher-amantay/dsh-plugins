...
export const inject = ['agents', 'goals', 'jobs', 'tools', 'systemPrompt', 'sessionProjections']
...
/** One process-local wake edge from a blocking job to the exact goal revision it suspended. */
...
function guidance(blockedAfter: number): string {
  return 'Use goal tools for one long-running completion objective in the current session. '
    ...
    + 'resume to rearm it. When a background job is the only remaining dependency, use '
    + 'defer_goal_until_job once instead of polling; its completion will rearm the exact suspended '
    + 'goal revision automatically. Mark complete only when the objective is actually achieved. Mark '
...
/** Validate and brand one model-supplied background-job id. */
function jobId(value: string): JobId {
  if (value.length === 0 || value !== value.trim()) {
    throw new HarnessError('job_id must be a non-empty trimmed string', 'GOAL_TOOL_INVALID_JOB')
  }
  return JobId(value)
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  const deferred = new Map<Agent, DeferredGoal>()

  function resumeDeferred(owner: Agent, wake: DeferredGoal, reason: string): void {
    try {
      const current = ctx.goals.get(owner)
      if (current === undefined
        || current.id !== wake.goal.id
        || current.revision !== wake.goal.revision
        || current.phase !== 'active'
        || current.activation !== 'disarmed') {
        deferred.delete(owner)
        return
      }
      ctx.goals.resume(owner, wake.goal)
      deferred.delete(owner)
    } catch (error: unknown) {
      const detail = error instanceof Error ? error.message : String(error)
      ctx.logger.warn(`tool-goal: could not resume goal ${reason}: ${detail}`)
    }
  }

  ctx.jobs.onJobDone((snapshot, owner) => {
    if (owner === undefined) return
    const wake = deferred.get(owner)
    if (wake === undefined || wake.jobId !== snapshot.id) return
    resumeDeferred(owner, wake, `after job "${snapshot.id}"`)
  })

  ctx.effect(() => () => {
    for (const [owner, wake] of deferred) resumeDeferred(owner, wake, 'during plugin disposal')
  }, 'tool-goal: release deferred goals')
  ...
  ctx.tools.register(defineTool({
    name: 'defer_goal_until_job',
    description: 'Suspend automatic rounds for the exact current active goal until one owned background job settles. '
      + 'Use this when that job is the only remaining dependency instead of polling it or pausing the goal. '
      + 'The job may complete, fail, or be killed; any terminal outcome rearms the same goal revision so the next round can inspect it. '
      + 'Execution requires a direct human turn or the exact current goal round.',
    parameters: {
      ...
      job_id: { type: 'string', required: true, description: 'Owned running background job that blocks further goal work.' },
    },
    output: GOAL_OUTPUT,
    execute(args, exec) {
      const execution = goalToolExecution(ctx, exec)
      ...
      if (current === undefined || current.id !== ref.id || current.revision !== ref.revision) {
        throw new HarnessError('goal_id and revision must match the exact current goal', 'GOAL_TOOL_STALE_UPDATE')
      }
      if (current.phase !== 'active' || current.activation !== 'armed') {
        throw new HarnessError('only an active, armed goal can be deferred', 'GOAL_TOOL_INVALID_DEFER')
      }
      if (deferred.has(execution.agent)) {
        throw new HarnessError('this goal already has a blocking background job', 'GOAL_TOOL_INVALID_DEFER')
      }
      const id = jobId(args.job_id)
      const job = ctx.jobs.get(id, execution.agent)
      if (job.status !== 'running') {
        throw new HarnessError(`job "${id}" is already ${job.status}`, 'GOAL_TOOL_INVALID_DEFER')
      }
      const goal = ctx.goals.disarm(execution.agent)
      if (goal === undefined) {
        throw new HarnessError('current goal disappeared before it could be deferred', 'GOAL_TOOL_STALE_UPDATE')
      }
      const wake = { jobId: id, goal: ref }
      deferred.set(execution.agent, wake)
      const latest = ctx.jobs.get(id, execution.agent)
      if (latest.status !== 'running') {
        resumeDeferred(execution.agent, wake, `after job "${id}" settled during deferral`)
      }
      return Promise.resolve(goalValue(ctx.goals.get(execution.agent)))
    },
    presentCall: args => present('Defer goal until job finishes', 'other', args.job_id),
  }))
