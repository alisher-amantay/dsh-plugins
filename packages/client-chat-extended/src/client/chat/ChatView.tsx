...
export function ChatView({
  ...
  const hasRunningToolCall = useChat(s => s.legacy.runningCalls.length > 0)
    ...
    <div className={css.root}>
      <div ref={listRef} className={css.scroll}>
        ...
        <div ref={columnRef} className={css.column} data-chat-flow="">
          ...
          {/* Model-activity signal spans request and streaming phases. A running
              tool already exposes the exact work, so the generic label yields. */}
          {running && !hasRunningToolCall && <TurnStatus startTime={runningTurnStart} t={t} />}
