...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  if (!Array.isArray(root.workspaces) || !root.workspaces.every(value => typeof value === 'string')) {
    throw new Error('tsdown: root package.json workspaces must be a string array')
  }
  const patterns = root.workspaces.map(pattern => `${pattern}/package.json`)
  for (const manifestPath of globSync(patterns, { cwd: REPOSITORY_ROOT })) {
